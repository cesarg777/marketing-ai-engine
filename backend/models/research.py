...
class ResearchProblem(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    week_id = Column(String(36), ForeignKey("research_weeks.id"), nullable=False)
    ...
    related_niches = Column(JSON, default=list)
    country = Column(String(5), nullable=False)
    language = Column(String(10), default="en")
    source_count = Column(Integer, default=0)
    source_urls = Column(JSON, default=list)
    suggested_angles = Column(JSON, default=list)
    keywords = Column(JSON, default=list)
    language_variants = Column(JSON, default=dict)
